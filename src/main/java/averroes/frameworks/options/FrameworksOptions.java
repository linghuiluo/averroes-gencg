...
public final class FrameworksOptions {
  ...
  public static String getSootClassPath() {
    ...
    String inputs = Paths.get("target", "test-classes").toString();
  ...
  /**
   * Get all classes under the Averroes classpath, given a package name prefix. This method is
   * useful when the input folder is not the root package folder.
   *
   * @param prefix
   * @return
   */
  ...
  private static List<String> getClassesUnder(String aPath, String prefix) { // This method calls
    // SourceLocator.v().getClassesUnder(aPath,
    // prefix)
    // which is not public accessible in
    // 3.3.0.
    try {
